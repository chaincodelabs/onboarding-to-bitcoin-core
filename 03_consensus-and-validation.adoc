...
Transactions can originate from the wallet, RPC commands, the P2P network or from tests.
They can be in the form of either a `CTransaction` or `CTransactionRef`, which is a shared pointer to a `CTransaction`.

.Transaction origination
[mermaid,target=tx-origination,format=svg,id=tx-origination]
....
flowchart LR
    process_tx["ProcessTransaction()"]
    ...
    process_msg --> process_block
    process_block --> check_block
    check_block --> check_transaction
...
.Curiosities surrounding P2P block validation
[sidebar]
****
...
Inside inner function `ProcessNewBlock()` we end up calling `CheckBlock()` twice: once on the outer `CheckBlock()` and once internally to `AcceptBlock()`
...
Having considered both transactions that have entered into the mempool, and any new transactions that were first learned about in the block itself, we now understand both ways a transaction can be deemed consensus-valid.

TIP: As you read through the following sub-sections, consider whether making changes to them could affect consensus, policy, or neither.

=== `AcceptToMemoryPool`

`AcceptToMemoryPool()` (ATMP) is where the checks on transactions occur before they enter the mempool.
...
flowchart TB
    ...
    classDef green fill:green,color:white,stroke:green;
...
Let's take a look inside ``AcceptToMemoryPool()``'s inner function `AcceptSingleTransaction()`:

.src/validation.cpp
[source,cpp,highlight=8;10;14;16,options=nowrap]
----
...
Once `AcceptSingleTransaction` has acquired the `cs_main` and `m_pool.cs` locks, initializes a `Workspace` (`ws`), which is essentially a storage area for (validation status) state which can be shared by the different validation checks, along with the struct of `ATMPArgs` passed in as argument.
...
The `Workspace` struct is initialized with a pointer to the transactions' `CTransactionRef` and holds some https://github.com/bitcoin/bitcoin/blob/v23.0/src/validation.cpp#L553-L593[additional] information related to intermediate state.

We can look at ATMPArgs https://github.com/bitcoin/bitcoin/blob/v23.0/src/validation.cpp#L469-L534[struct] to see what other information our mempool wants to know about in addition to transaction information.

.A look at some key ATMPArgs
[sidebar]
****
...
If the various checks pass, and this was not a test submission, then we will `Finalize` (`MemPoolAccept::Finalize`) the transaction, adding it to the mempool, before trimming the mempool size and updating any affected RBF transactions as required.
...
The `PreChecks` https://github.com/bitcoin/bitcoin/blob/v23.0/src/validation.cpp#L668-L906[function] is very long but is worth examining to see which checks are undertaken at this stage.
