...
You can look at the `bitcoin.conf` files within the data directories to see what other config options are being specified (there's always `-regtest=1`), which is a good way to learn advanced uses of regtest.
...
This has the further advantage (over calling `sleep()`) that you can single-step through the test while also manually interacting with the nodes, combining automated and manual testing.
