...
NOTE: This document was written against an older version of Bitcoin Core. Specific code references and file paths may be outdated, but the high-level architectural concepts remain relevant.
...
ryanofsky has written a handy https://github.com/bitcoin/bitcoin/blob/master/doc/design/libraries.md[guide^] covering the different libraries contained within Bitcoin Core, along with some of their conventions and a dependency graph for them.
