...
|unit tests
|`ctest --test-dir build` or `./build/bin/test_bitcoin`
...
Bitcoin Core also has (functional) "stress tests" which challenge the program via interruptions and missing files to ensure that we fail gracefully, e.g. the tests introduced in https://github.com/bitcoin/bitcoin/pull/23289[PR#23289^].
...
NOTE: The `fuzz_targets` themselves are located in `src/test/fuzz/`, while the fuzz `test_runner` is located in `test/fuzz/`.

NOTE: `qa_assets` are found in a https://github.com/bitcoin-core/qa-assets[separate^] repo altogether, as they are quite large (~0.6GB repo size and ~6.9GB on clone).
