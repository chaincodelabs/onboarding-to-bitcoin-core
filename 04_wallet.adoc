...
.Wallet database
[mermaid,target=wallet-database,format=svg]
....
flowchart BT
   database["database\n(KV store)"]
...
These are some of the key https://github.com/bitcoin/bitcoin/blob/master/src/wallet/walletdb.cpp#L30-L62[records] which help us regenerate a descriptor wallet from the database:
...
For `LegacyScriptPubKeyMan` (LSPKM) we use the records with `*.KEY` & `SCRIPT`.

Wallet metadata includes `tipLocator`, the most recent tip, and `version` which is used in database upgrades.
...
[TIP]
====
`cKey` = encrypted ("crypted") private key

`Key` = unencrypted private key
====

The various `scriptPubkeyMan` objects are owned by the `CWallet` instance eventually, however `LegacyScriptPubKeyMan` is both created and owned by `CWallet`, whereas `DescriptorScriptPubKeyMan` (DSPKM) is created externally to `CWallet`, and only after loading exists in the `CWallet` context.

Note that `TxSpends` is **not** tracked in the database (and loaded), but instead is rebuilt from scratch at startup because it's fast to do so.
Also, we must reload every transaction anyway so it's not much more work to regenerate `TxSpends` while we do this.
////
...
`CWallet` stores `CMasterKey` (**not** `masterPrivateKey`) which is actually our encryption key (not our master key).
The encryption key is the key we use to encrypt the (master) private key.

`CMasterKey` is what is encrypted by the user's passphrase.
If you change your passphrase, you are only changing the encryption applied to the `CMasterKey`.

`CWallet` has a map of ``CMasterKey``s and when a wallet unlock is performed it will try each one to see if it can decrypt and then unlock the wallet.
...
CAUTION: Decrypting the Bitcoin Core wallet requires the user to enter their passphrase, so is not convenient to do at every new block.

When **en**crypting a wallet, an encryption key is generated, which is then sent to the `ScriptPubKeyMan` to encrypt using its `.Encrypt()` method.

Once the wallet is encrypted for the first time, we re-generate all of our keys.
This means a new HD seed for `LegacyScriptPubKeyMan` and 8 new descriptors for `DescriptorScriptPubKeyMan`.
This is to avoid the wallet using things which were not "born encrypted" in the future.

If the wallet has already been used before, old ``ScriptPubKeyMan``'s **are** retained, and so remain usable, but are not marked as `active`.
The wallet will switch to the new SPKM after encryption.
...
Therefore we use some hacks so that when we request deletion of unencrypted private keys from the DB, they are properly deleted immediately and not "marked as" deleted.
====
...
TIP: `COutPoint` is a pair of `txid : index`, good when you want to know which UTXO an input spends.

``COutput``s (**not** `COutPoint`) are created for coin selection and contain the entire previous UTXO (script, amount), along with helpers for calculating fees and effective value.
...
For DSPKM running `IsMine()` is really simple.
Descriptors generate a list of ScriptPubKeys, and, if it's in the list, then it's `IsMine`.
...
LSPKM can have watch-only and spendable flags set at the same time, but DSPKM is either or.
Because LSPKM wallets are all key-based, we will need to see if a script _could have been generated by one of our keys_; what type of script it is, and if we have a (private) key for it?

For watch-only we simply check "do we have this script stored as a script?" (where db `CScripts` are our watch-only scripts)".
If we don't have a key for a script but it exists in `mapScripts` then it's implicitly watch-only.

The problem with this current method of `IsMine` is that it's tough to figure out what your wallet considers "Mine" -- it's probably a finite set, but maybe not...
Another consideration along this same vein is that `IsMine` includes P2PK outputs which don't have addresses.
...
See https://bitcoinops.org/en/topics/coin-selection/[Bitcoin Optech] for more information on what coin selection is.
There is also a section digging deeper into coin selection found <<coin-selection,below>>.

Iterate `mapWallet` and check for ismine, notspend and notconflicted, (conf) depth, immature coinbase stored in `availabelCoints`, list of `COutputs`.

`GroupOutputs()` takes those and turns them into OutputGroups. All outputs with the same script are counted as a signle Output by this function.
Also calculate values, fees, and dependatnt 
