...
$DATADIR/blocks/xor.dat::
Rolling XOR pattern used to obfuscate block and undo data files (prevents antivirus false positives on block data).

=== Configuration and log files

$DATADIR/bitcoin.conf::
Configuration file for Bitcoin Core. Contains runtime options like network settings, RPC credentials, and node behavior. If not present, command-line options or defaults are used.

$DATADIR/settings.json::
Stores GUI settings and runtime configuration options (e.g., prune mode settings, loaded wallets). Automatically managed by Bitcoin Core and takes precedence over `bitcoin.conf` for overlapping settings.

$DATADIR/debug.log::
Main log file containing node activity, warnings, and errors. Useful for troubleshooting. Can be tailed with `tail -f ~/.bitcoin/debug.log` on Unix systems.
...
Tx Spender Index::
Filesystem location of transactions by the outputs they spend
...
|Tx Spender Index
|_$DATADIR/indexes/txospenderindex_
|Yes
|`TxSpenderIndex`
...
|Coinstats Index
|_$DATADIR/indexes/coinstatsindex/db/_
|Yes
|`CoinstatsIndex`
