...
See <<CTxMemPoolEntry>> for more information on what data counts towards this limit, or review the `CTxMemPool` data members which store current usage metrics e.g. `CTxMemPool::cachedInnerUsage` and the implementation of e.g. `CTxMemPool::DynamicMemoryUsage()`.
...
These describe mempool entries (i.e. transactions) in the mempool.
...
Another point to note is that some of this extra transaction metadata *does* count towards the mempool's maximum size, therefore a default mempool of 300MB will contain less than 300MB of serialised transactions.
...
Digging further into `MemPoolAccept::AcceptSingleTransaction()` we can see where the primary policy and validation checks on transactions occur before the enter the mempool.
...
If the various checks pass, and this was not a test `MemPoolAccept`, then we will `Finalize` (`MemPoolAccept::Finalize`) the transaction, adding it to the mempool, trimming the mempool size and updating and affected RBF transactions as required.
...
ifdef::env-github[image::block.svg[]]
