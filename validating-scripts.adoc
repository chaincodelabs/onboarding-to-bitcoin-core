...
For some additional context on how scripts are validated in Bitcoin see <<{script-appendix}executing_scripts>> in the Appendix.
...
When a new transaction is learned about from the wallet or P2P network (as a TX INV ) it is passed to <<{single-transactions}single_transactions, `AcceptToMemoryPool()`>> which will run the various script checks.
