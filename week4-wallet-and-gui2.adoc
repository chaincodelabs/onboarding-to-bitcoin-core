...
:toc:
:toclevels: 4
:src: https://github.com/bitcoin/bitcoin/blob/4b5659c6b115315c9fd2902b4edd4b960a5e066e/src
:obc: https://github.com/chaincodelabs/bitcoin-core-onboarding/blob/main
:devwiki: https://github.com/bitcoin-core/bitcoin-devwiki/wiki/
:wallet: https://github.com/bitcoin/bitcoin/blob/4b5659c6b115315c9fd2902b4edd4b960a5e066e/src/wallet
...
* {obc}/1.1_regions.asciidoc#wallet_region[Bitcoin core onboarding - wallet/] describes the main functions of a wallet, along with some of the differences between legacy and descriptor wallets. 
...
The {devwiki}/Process-Separation[Process Separation] project is tracking development working towards separating out node, wallet and GUI code even further.
...
* The node holds a {wallet}/interfaces.cpp#L109[`WalletImpl`] interface to call functions on the wallet.
* The wallet holds a {src}/node/interfaces.cpp#L429[`ChainImpl`] interface to call functions on the node.
* The node notifies the wallet about new transactions and blocks through the {src}/node/interfaces.cpp#L341[`CValidationInterface`].
...
The wallet component is initialised through the `WalletInitInterface` class which is specified in {src}/walletinitinterface.h#L11-L23[src/walletinitinterface.h].
...
The src tree's {src}/Makefile.am#L362-L367[Makefile] describes which of these modules is chosen to override: if `./configure` has been run with the wallet enabled (by default), then `wallet/init.cpp` is added to the sources, otherwise `dummywallet.cpp` is added.

The wallet interface is constructed when `Construct` is called on the `g_wallet_init_interface` object by the {src}/init.cpp#L1180-L1189[`AppInitInterfaces` function in init.cpp].
...
The wallet is first verified (before being started) as part of `AppInitMain` which first {src}/init.cpp#L1301-L1305[verifies wallet database integrity] by calling {wallet}/load.cpp#L19-L88[VerifyWallets] via the `WalletClientImpl` override of `client->verify`.
...
The wallet(s) are loaded  when `client->load` is called on each `node.chain_client` as part of {src}/init.cpp#L1728-L1732[init.cpp].
...
The call to  `load` on the wallet `chain_client` has again been overridden, this time by ``WalletClientImpl``'s {wallet}/load.cpp#L90-L121[`LoadWallets` method].
This function works similarly to `VerifyWallets`, first creating the `WalletDatabase` for each wallet (although this time skipping the verify step) before creating a `CWallet` object from the wallet database object and adding it to the global list of wallets, the vector `vpwallets`, by calling {wallet}/load.cpp#L114[`AddWallet`].
`AddWallet` is defined in {wallet}/wallet.cpp#L98-L108[src/wallet.cpp].

=== Startup

The wallet is finally "started" when (all) `chain_clients` are started in {src}/init.cpp#L1939-L1941[init.cpp] which calls the overridden `client->start` method from the `WalletClientImpl` class, resulting in {wallet}/load.cpp#L123-L134[src/wallet/load.cpp::StartWallets] being called.
...
Also as part of `StartWallets` `flushwallet` might be scheduled (if configured by argument) and wallet transactions are scheduled to be resent every second (although {wallet}/wallet.cpp#L2104-L2147[delayed upstream with a random timer]).
...
If using the native GUI from within `bitcoin`(-qt) then communication with the wallet is done directly via qt's {src}/qt/walletmodel.h#L50-L242[`WalletModel` interface].

Commands which can be used to control the wallet via RPC are listed in {wallet}/rpcwallet.cpp#L4584-L4657[rpcwallet.cpp].

=== Wallet via RPC

If we take a look at the {wallet}/rpcwallet.cpp#L2562-L2620[`loadwallet` RPC] we can see similarities to ``WalletClientImpl``'s `LoadWallets` function.
...
Next it will call {wallet}/wallet.cpp#L237-L248[`wallet.cpp#LoadWallet`] which starts by grabbing `g_wallet_loading_mutex` and adding the wallet to `g_loading_wallet_set`, before calling {wallet}/wallet.cpp#L207-L234[`LoadWalletInternal`] which adds the wallet to `vpwallets` and sets up various event notifications.
...
The document {src}/doc/descriptors.md[Support for Output Descriptors in Bitcoin Core] provides more details and examples of these output descriptors.

==== ScriptPubkeyManager

* Each wallet can now contain multiple {wallet}/scriptpubkeyman.h#L169[``ScriptPubkeyManager``s], who are in control of managing the ``scriptPubkey``s in a wallet.
...
When a new transaction is learned about (either entering into the mempool or in a new block) the wallet is notified through the {src}/node/interfaces.cpp#L341[`CValidationInterface`].
This will call the function {wallet}/wallet.cpp#L1182[`CWallet:SyncTransaction()`] which will in turn call {wallet}/wallet.cpp#L1015[`CWallet::AddToWalletIfInvolvingMe()`].
...
* There is useful documentation for developers looking to contribute to the QT side of the codebase found at {devwiki}/Developer-Notes-for-Qt-Code[Developer Notes for Qt Code].
