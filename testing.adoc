...
In order to run the lints yourself you'll have to first make sure your python environment and system have the packages listed in the CI install https://github.com/bitcoin/bitcoin/blob/v30.2/ci/lint/04_install.sh[script].
...
NOTE: The lint checks have been refactored over different Bitcoin Core versions. In v30.2, individual Python lints can be run separately, or you can run the full CI lint suite. Older versions (pre-v24) used shell scripts (`*.sh`).
